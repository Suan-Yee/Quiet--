    ...
    <Card className="p-6">
      ...
      <div className="mt-5 space-y-3">
        {suggestedAuthors.map((author) => (
          <div
            key={author.name}
            className="flex gap-3 rounded-2xl p-2 transition hover:bg-[var(--color-card-elevated)]"
          >
            <img
              src={author.avatar}
              alt=""
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[var(--color-text)]">{author.name}</p>
                  <p className="text-xs font-medium text-[var(--color-muted)]">{author.topic}</p>
                </div>
                <button
                  type="button"
              ...
              <p className="mt-2 text-xs leading-5 text-[var(--color-secondary)]">{author.description}</p>
            </div>
          </div>
        ))}
      </div>
